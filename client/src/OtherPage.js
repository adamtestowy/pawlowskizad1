...
export default () => {
  return (
    <div>
      ...
      <p>Jako strona domowa pierwsza wyswietla sie kalkulator Fibonacciego. Kolejno podstrona Home prowadzi do strony glownej (2 informacje) oraz podstrona Other Page (do opisu technicznego uslugi). 
...
Postgres: Przechowa liste z wspolczynnikami ciagu Fibonacciego.</p>
      <p>Usluga jest przygotowana na etapie rozwoju (dev: docker-compose.dev) 

Usluge uruchamiamy poleceniem: docker compose -f dockercompose.dev.yml up build

Port z 3050 mozna zmienic na inny np. 3060 w pliku: docker-compose.dev.yml w sekcji nginx ports.</p>
      <p>Punkty extra:
...
5) Nastepnie budowane sa obrazy produkcyjne dla: client/nginx/server/worker, logowanie do dockerHub za pomoca zdefiniowanych zmiennych srodowiskowych w Travis (DOCKER_ID I DOCKER_PASSWORD) I kolejno poloeceniem docker push wrzucane sa 4 obrazy do konta na dockerHub.
...
Niestety nie bylem w stanie uruchomic triggera na Travis, poniewaz na moim koncie w travis skonczyl sie miesiecznny dostep (informacja na czerwono). </p>
